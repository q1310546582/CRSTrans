...
class XYZ_3p(GxXYZTransAsPublicClass):
    ...
    def __init__(self, public_points):
        '''
        空间直角三参数模型转换模型参数说明：
        参数1支持公共点的浮点型二维数组，其中数量不少于1个，并且每个元素存储原坐标系坐标和新坐标系坐标(X1,Y1,Z1,X2,Y2,Z2)
        '''

        self.public_num = 1  # 至少要求的公共点数量
        self.dimension = 3  # 当前模型坐标的维度
        self.model_name = 'Three parameter transformation model'
    ...
    def fit(self):
        ...
        ''')
        return {'model': self.model_name,  'x': {'OffsetX': self.__prams_array[0], 'OffsetY': self.__prams_array[1], 'OffsetZ': self.__prams_array[2]} ,
...
if __name__ == '__main__':
    ...
    xyz_3p = XYZ_3p(public_array)
    dic = xyz_3p.fit()
    print(xyz_3p.Predict_points)
    print(xyz_3p.residual_array)